...
author: "Loyal A. Goff & Genevieve Stein-O'Brien"
date: "7/10/2021"
...
path<-'/mnt/morbo/Data/Users/lgoff/Dolen/psychadelic_study_NAb/analysis/NAb_psychadelics'
setwd(path)
...
s2c <- read_csv("/mnt/morbo/Data/Users/lgoff/Dolen/psychadelic_study_NAb/metadata/BulkRNAPsychodelics_5.25.2021.csv")
s2c$path<-paste0("/mnt/morbo/Data/Users/lgoff/Dolen/psychadelic_study_NAb/kallisto/",s2c$sample)
...
#so2<-readRDS("/mnt/morbo/Data/Users/lgoff/Dolen/psychadelic_study_NAb/analysis/NAb_psychadelics/cache/psychadelic_study_NAb_day2only.rds")
...
#genes_LRR_treated<-genes_summary_treatment_treated %>%
#  group_by(target_id,treatment) %>%
#  summarise(ratio = mean/mean[treatment=="saline"], se.lRR=sqrt((se**2/mean**2)+(se[treatment=="saline"]**2/mean[treatment=="saline"]**2)))



# #TPM Barplots
# pdf("plots/FigureS4/sigGenes_barplots_saline_vs_treated.pdf",width=6,height=3)
# for(gene in sleuth_significant_treated$target_id){
#   message(gene)
#   #gene<-sleuth_significant$target_id[1]
#   p<-ggplot(genes_summary_treated[genes_summary_treated$target_id == gene,],aes(x=days,y=mean)) +
#     geom_bar(aes(fill=treatment),stat="identity",color="black") +
#     geom_errorbar(aes(ymin=mean-sd,ymax=mean+sd),color="black",width=0.5) + 
#     geom_point(aes(x=days,y=tpm,fill=treatment),data=genes_to_plot_treated[genes_to_plot_treated$target_id == gene,],color="black",shape=21,size=2) +
...
#TPM ratio to saline control
#TODO: if this is the preferred visualization, then we need to figure out how to appropriately propogate errors for error bars when estimating the TPM ratio to saline mean...
...
# Psychadelics vs Non-Psychadelics
This section is deprecated and not included in the manuscript currently (8/4/21)
<!-- ```{r} -->
<!-- so<-sleuth_fit(so, ~rna_isolation_batch+psychoactive, 'reduced_psychoactive') -->
<!-- so <- sleuth_lrt(so,'reduced','reduced_psychoactive') -->
<!-- ``` -->

<!-- ```{r psychoactive_prelim_results} -->
<!-- qval<-0.2 -->
<!-- sleuth_table_psychoactive <- sleuth_results(so, 'reduced:reduced_psychoactive', 'lrt', show_all = FALSE) -->
<!-- sleuth_table_psychoactive <- sleuth_table_psychoactive[!is.na(sleuth_table_psychoactive$pval),] #valid tests only -->
<!-- #head(kable(sleuth_table_critical_period)) -->
<!-- sleuth_significant_psychoactive <- sleuth_table_psychoactive[sleuth_table_psychoactive$qval <= qval,] -->
<!-- dim(sleuth_significant_psychoactive)[1] -->
<!-- kable(sleuth_significant_psychoactive) -->
<!-- ``` -->

<!-- ```{r FigS6_lrt_heatmap} -->
<!-- #genes<-sapply(FigS5_sleuth_significant$target_id, function(x) FigS5_sleuth_significant[FigS5_sleuth_significant$target_id==x,"ext_gene"]) -->

<!-- figS6<-plot_transcript_heatmap_mod(so,sleuth_significant_psychoactive, 'scaled_reads_per_base', cluster_transcripts = TRUE, -->
<!--                         annotation_cols = c('treatment','rna_isolation_batch',"critical_period","psychoactive"), scale = "row", -->
<!--                         color_high="yellow",color_mid="black",color_low="blue", -->
<!--                         main=paste0("All samples psychoactive L.R.T q<",qval), #labels_row = genes,  -->
<!--                         clustering_distance_col="correlation" -->
<!--                         ) -->
...
<!-- pdf("plots/FigureS6/LRT_heatmap_psychoactive.pdf",width=7,height=10) -->
<!-- plot_transcript_heatmap_mod(so,sleuth_significant_psychoactive, 'scaled_reads_per_base', cluster_transcripts = TRUE, -->
<!--                         annotation_cols = c('treatment','rna_isolation_batch',"critical_period","psychoactive"), scale = "row", -->
<!--                         color_high="yellow",color_mid="black",color_low="blue",show_rownames=FALSE, -->
<!--                         main=paste0("All samples psychoactive L.R.T q<",qval), #labels_row = genes,  -->
<!--                         clustering_distance_col="correlation" -->
...
<!-- ## TPM plots for psychoactive -->
<!-- ```{r} -->
<!-- genes_to_plot_psychoactive<-so$obs_norm[so$obs_norm$target_id %in% sleuth_significant_psychoactive$target_id,] -->
<!-- genes_to_plot_psychoactive<-merge(genes_to_plot_psychoactive,s2c,by="sample",all.x=TRUE) -->

<!-- genes_summary_psychoactive<-genes_to_plot_psychoactive %>% -->
<!--   group_by(target_id,treatment,days) %>% -->
<!--   summarise(mean=mean(tpm), sd=sd(tpm)) -->

<!-- genes_summary_treatment_psychoactive<-genes_to_plot_psychoactive %>% -->
<!--   group_by(target_id,treatment) %>% -->
<!--   summarise(mean=mean(tpm), sd=sd(tpm)) -->
...
<!-- pdf("plots/FigureS6/sigGenes_barplots_log_ratio_to_saline_psychoactive.pdf",width=6,height=3) -->
<!-- for(gene in sleuth_significant_psychoactive$target_id){ -->
<!--   message(gene) -->
<!--   #gene<-sleuth_significant$target_id[1] -->
<!--   p<-ggplot(genes_summary_psychoactive[genes_summary_psychoactive$target_id == gene,],aes(x=days,y=log(mean/genes_summary_treatment_psychoactive$mean[genes_summary_treatment_psychoactive$target_id==gene & genes_summary_treatment_psychoactive$treatment == "saline"]))) + -->
<!--     geom_bar(aes(fill=treatment),stat="identity",color="black") + -->
<!--     #geom_errorbar(aes(ymin=mean-sd,ymax=mean+sd),color="black") +  -->
<!--     geom_point(aes(x=days,y=log(tpm/genes_summary_treatment_psychoactive$mean[genes_summary_treatment_psychoactive$target_id==gene & genes_summary_treatment_psychoactive$treatment == "saline"]),fill=treatment),data=genes_to_plot_psychoactive[genes_to_plot_psychoactive$target_id == gene,],color="black",shape=21,size=2) + -->
<!--     scale_fill_manual(values=match_col[c(5,1,2,3,4)]) + -->
<!--     ggtitle(sleuth_significant_psychoactive$ext_gene[sleuth_significant_psychoactive$target_id == gene]) + -->
<!--     monocle3:::monocle_theme_opts() +  -->
<!--     geom_hline(yintercept = 0,linetype="solid") + -->
<!--     xlab("Treatment") + -->
<!--     ylab("TPM Ratio to Saline Mean") + -->
